...
The primary release file is \file{data/student_visa.sqlite}. The database, common CSV, raw sources, source-level CSVs, and validation audit are distributed as a separate data artifact (DOI forthcoming), rather than committed to the GitHub code repository \cite{repository}. The code repository contains the scripts, machine-readable source inventory at \file{data/country-schema.json}, retrieval documentation at \file{data/RETRIEVAL.md}, and executable workflow description at \file{scripts/collect_data_script_chain.json}. The data files can be obtained from the artifact or recreated with the documented retrieval and build scripts.
